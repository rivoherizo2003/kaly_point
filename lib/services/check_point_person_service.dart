  ...
  Future<dynamic> fetchServedPersons({
    ...
    try {
      ...
      final servedPersons = await db.rawQuery(
        'SELECT p.id AS person_id, p.lastname ,p.firstname,cpp.check_point_id, cpp.id AS check_point_person_id FROM person p JOIN check_point_person cpp ON cpp.person_id  = p.id WHERE cpp.check_point_id  = ? ORDER BY p.lastname ASC',
        [checkPointId],
      );
